...
def adb_logcat():
    ...
    time.sleep(0.5)
    for i in devices_list:
        cmd = 'adb -s ' + i + fr' shell dumpsys dropbox --print >%s\crash{i}.txt' % pathss
        print("正在执行:", cmd)
        os.popen(cmd)
...
def adb_maxim(pushjar):
    ...
    for i in devices_list:
        maxin = 'start adb -s ' + i + ' shell "CLASSPATH=/sdcard/monkey.jar:/sdcard/framework.jar exec app_process /system/bin tv.panda.test.monkey.Monkey -p ' + apk[j] + '  --uiautomatormix --running-minutes {0} -v -v'.format(
            times)
        with open(file_path + f'\\{i}start.bat', 'w+') as f:
            f.write(maxin)
        os.system(file_path + f'\\{i}start.bat')
        os.popen('del ' + file_path + f'\\{i}start.bat')
        if len(apk) > 1:
            j = j + 1
...
def download_install():
    ...
    if os.path.exists(file_name):
        print("安装包准备就绪正在安装...")
        cmd = r'adb install -r {0}\{1}'.format(file_path, file_name)
        install = os.popen(cmd)
        ...
        for line in install_txt.splitlines():
            if line == "Success":
                ...
                print("安装失败：", line)
                print(r"可扫码安装：{}\{}".format(file_path, image_name))
                # 安装失败将url转换成二维码图片
                qr = qrcode.QRCode(
                    version=1,
                ...
                if os.path.exists(image_name):
                    show_image = Image.open(image_name)
                    show_image.show()
                else:
                    print("图片不存在！")
